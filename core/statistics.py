...
from pprint import pprint # so that, under Python 2.*, integer division results in real numbers
from .util import *
...
import pandas as pd
import scipy
# from statsmodels.stats.proportion import test_proportions_2indep, confint_proportions_2indep,power_proportions_2indep
...
def make_delta(assume_normal=True, alpha=0.05, min_observations=20, nruns=10000, relative=False, endpoint='normal'	):
    """ A closure to the delta function. """
    def go(x, y, x_denominators=1, y_denominators=1, endpoint='normal'):
        return delta(x, y, x_denominators, y_denominators, assume_normal, alpha, min_observations, nruns, relative, endpoint='normal')
    return go


def delta(x, y, x_denominators=1, y_denominators=1, assume_normal=True, alpha=0.05, min_observations=20, nruns=10000, relative=False , endpoint='normal'):
    ...
    statistical_power = compute_statistical_power_from_samples(_x_strange, _y_strange, alpha) # this is a bit rubbish, only applies to bootstrap and min_observations
    test_statistic       = compute_test_statistic_from_samples(_x_strange, _y_strange)
    ...
    return SimpleTestStatistics(variant_statistics.control_statistics,
                                variant_statistics.treatment_statistics,
                                 delta = float(mu), ci=    c_i, p = p_value, test_statistic = test_statistic, statistical_power = statistical_power, alpha= alpha)
...
def estimate_sample_size(x, mde, r, alpha=0.05, beta=0.2, endpoint = 'normal'):
    """
    ...
    if endpoint == 'binary':
        p_control = x.sum()/len(x)
        p_treatment = p_control + mde
        effect_size = sms.proportion_effectsize(p_control, p_treatment)
        sample_size = sms.NormalIndPower().solve_power(effect_size, power = 1- beta, alpha = alpha, ratio = r)
...
def normal_difference(mean1, std1, n1, mean2, std2, n2, percentiles=[2.5, 97.5], relative=False, equal_var=True):
...
def welchs_ttest_ci(x,y,percentiles=[2.5,97.5]):
    c_l = percentiles[1]/100-percentiles[0]/100 
    confint = stats.ttest_ind(x, y, equal_var = False).confidence_interval(c_l)
    return { percentiles[0]:  confint[0],  percentiles[1]: confint[1] }
         

def two_proprotions_confint(success_treatment, size_treatment, success_control, size_control,alpha=0.05,relative=False,percentiles=[2.5, 97.5]):
  
    from statsmodels.stats.proportion import confint_proportions_2indep

    confint = confint_proportions_2indep(count1 = success_treatment, nobs1 = size_treatment, count2 = success_control, nobs2 = size_control, method=None, compare='diff', alpha=alpha, correction=True)

    return dict( {percentiles[0]:  confint[0] ,percentiles[1]:  confint[1] })




def compute_statistical_power_from_samples(x, y, alpha=0.05, endpoint =  'normal'):
    ...
    if endpoint ==  'binary':
        ...
        p_y = sum(_y)/n2 #P2     
        diff = p_x-p_y
        return power_proportions_2indep(diff, p_x, n2, ratio=n2/n1, alpha=alpha, value=0, alternative='two-sided', return_results=False)
...
def compute_p_value_from_samples(x, y, endpoint='normal' ,equal_var=False):
    ...
    if endpoint == 'binary':
        ...
        count1 = sum(x)  #P1
        count2 = sum(y)  #P2     
        test_statistic, p_value = compute_test_statistic_p_value_proportions(count1 = count1, nobs1 = nobs1, count2 = count2, nobs2 = nobs2)
        return p_value
    ...
    if equal_var==True:
        return compute_p_value(mean1, std1, n1, mean2, std2, n2,equal_var=True)
    elif equal_var==False:
        t,p=stats.ttest_ind(_x, _y, equal_var = False)
        return p
    else:
        raise ValueError('equal_var must be True or False')


def compute_p_value(mean1, std1, n1, mean2, std2, n2,equal_var=True):
    ...
    if equal_var==True:
        ...
        raise ValueError('equal_var must be True')
 
def compute_test_statistic_from_samples(x, y, endpoint='normal',equal_var=False):
    """ Calculates test statistic for statistical Student's T-test based on pooled standard deviation.
    ...
    if equal_var==True:
        return compute_test_statistic(x1_mean,x2_mean,std_diff)
    elif equal_var == False:
...
def compute_test_statistic(x1_mean, x2_mean, std_diff,diff=0):
    """ Compute test statistic for statistical Student's T-test given statistics of control and treatment.
    
    :param x1_mean: mean value of the treatment distribution
    :type  x1_mean: float
    :param x2_mean: mean value of the control distribution
    :type  x2_mean: float
    :param std_diff: standard deviation of the difference distribution
    :type  std_diff: float
    :param diff: difference between means
    :type  diff: float
    
    :return: test statistic
    :rtype: float
    """
    return (x1_mean - x2_mean - diff) / std_diff
...
def compute_test_statistic_p_value_proportions(count1, nobs1, count2, nobs2):
    """ Computes test statistic and p-value for two proportions test.

    :param count1: number of successes of first sample
    :type  count1: int
    :param nobs1: total number of observations of first sample
    :type  nobs1: int
    :param count2: number of successes of second sample
    :type  count2: int
    :param nobs2: total number of observations of second sample
    :type  nobs2: int
    ...
    # This function might offer more flexibility and could be more robust in handling cases with low proportions or rare events.
    return test_proportions_2indep(count1, nobs1, count2, nobs2, alternative='two-sided', method='score', correction=True, return_results=False)
    

def sequential_sampling(alpha, power_level, p, delta):
    """Given a desired power level, alpha, p and delta, this function returns the number of samples needed for a sequential test. 
    The function uses a binary search to find the number of samples needed for a sequential test. 

    :param alpha: significance level
    :type  alpha: float
    :param power_level: desired power level
    :type  power_level: float
    :param p: probability of null hypothesis
    :type  p: float
    :param delta: minimum detectable effect
    :type  delta: float
    
    :return: number of samples needed for a sequential test
    :rtype: int
    """
    MAX_BARRIER = 5000
    null_p = 0.5
    alt_p = 1.0/(1.0+(p+delta)/p)
    ...
    z_lo = 1
    z_hi = MAX_BARRIER
    best_odd_z,z_hi = bsearch(1, MAX_BARRIER-1, alpha, power_level, null_p, alt_p)
    best_even_z,z_hi = bsearch(2, MAX_BARRIER, alpha, power_level, null_p, alt_p)
    odd_n = n_for_z(best_odd_z, alpha, power_level, null_p, alt_p)
    even_n = n_for_z(best_even_z, alpha, power_level, null_p, alt_p)
    if (math.isnan(odd_n) | even_n < odd_n):
        number_ahead = best_even_z
        return even_n,number_ahead
    
    number_ahead = best_odd_z
    return odd_n,number_ahead    