...
from .statistics import sample_size,normal_difference,compute_p_value_from_samples,compute_statistical_power_from_samples,two_proprotions_confint,compute_test_statistic_from_samples, welchs_ttest_ci
...
def make_group_sequential(spending_function='obrien_fleming', estimated_sample_size=None, alpha=0.05, cap=8,endpoint='normal'):
    """ A closure to the group_sequential function. """

    def go(x, y, x_denominators=1, y_denominators=1,endpoint=endpoint):

        if endpoint == 'binary':
            return group_sequential_binominal(x, y, spending_function, estimated_sample_size, alpha, cap)
...
def group_sequential(x, y, spending_function='obrien_fleming', estimated_sample_size=None, alpha=0.05, cap=8,printout=False, endpoint='normal'):
    ...
    if endpoint == 'normal':
        if  isinstance(x.sum(), type(np.float64())) == False |  isinstance(y.sum(),type(np.float64())) == False:
            raise TypeError('Please provide samples of type Normal.')        
    elif endpoint == 'binary':

        if  isinstance(x.sum(), type(np.int64())) == False |  isinstance(y.sum(),type(np.int64())) == False:
            raise TypeError('Please provide samples of type Binary.')     
    else:
        raise TypeError('Please provide endpoint = "normal" or "binary". ')        
    ...
    logging.info("Information fraction is {}".format(information_fraction))
    ...
    logging.info("alpha_new is {}".format(alpha_new))
    ...
    logger.info("bound is {}".format(bound))
    if z > bound or z < -bound:
        stop = True
    else:
        stop = False

    interval = welchs_ttest_ci(_x,_y,percentiles=[alpha_new * 100 / 2, 100 - (alpha_new * 100 / 2)])
    ...
    p_value              = compute_p_value_from_samples(_x, _y, equal_var = False)
    alpha              = alpha_new
    test_statistic       = compute_test_statistic_from_samples(_x, _y, equal_var = False)
    statistical_power    = compute_statistical_power_from_samples(_x, _y, alpha_new)
    ...
    return EarlyStoppingTestStatistics(variant_statistics.control_statistics,
                                       ...
                                       float(mu_x - mu_y), interval, p_value, alpha, test_statistic, statistical_power, stop)


def group_sequential_binominal(x, y, spending_function='obrien_fleming', estimated_sample_size=None, alpha=0.05, cap=8,printout=False, endpoint='binary'):
    """ Group sequential method to determine whether to stop early.
    ...
    logger.info("group_sequential_binominal")
    ...
    if endpoint == 'normal':
        ...
        raise TypeError('Please provide endpoint = "normal" or "binray". ')        
    ...
    if  np.isnan(z):
        raise ValueError("z cannot be null, np.nan, None")
    logger.info("z is {}".format(z))
    logger.info("bound is {}".format(bound))
    if z > bound or z < -bound:
        stop = True
    ...
    interval = two_proprotions_confint(success_control = sum(_y), size_control = n_y, success_treatment=sum(_x), size_treatment=n_x,alpha=alpha_new, percentiles=[alpha_new * 100 / 2, 100 - (alpha_new * 100 / 2)])

    treatment_statistics = SampleStatistics(int(n_x), float(np.nanmean(_x)), float( p_x*(1-p_x) ))
    control_statistics   = SampleStatistics(int(n_y), float(np.nanmean(_y)), float( p_y*(1-p_y) ))
    variant_statistics   = BaseTestStatistics(control_statistics, treatment_statistics)
    p_value              = compute_p_value_from_samples(_x, _y,endpoint='binary')
    statistical_power    = compute_statistical_power_from_samples(_x, _y, alpha_new, endpoint='binary')
    test_statistic       = compute_test_statistic_from_samples(_x, _y,endpoint='binary')
    ...
    return EarlyStoppingTestStatistics(variant_statistics.control_statistics,
                                       variant_statistics.treatment_statistics,
                                       float(p_x - p_y), interval, p_value,alpha_new, test_statistic, statistical_power, stop)
