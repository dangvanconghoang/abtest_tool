...
class Experiment(object):
    ...
    def analyze_statistical_test(self, test, test_method='fixed_horizon', endpoint='normal',include_data=False,  **worker_args):
        ...
        # run the test method
        test_statistics = worker(x=treatment_numerators, y=control_numerators, x_denominators = treatment_denominators, y_denominators = control_denominators, endpoint = endpoint)
    ...
    def analyze_statistical_test_suite(self, test_suite, test_method='fixed_horizon' , endpoint = 'normal', **worker_args):
        ...
        for test in test_suite.tests:
            original_analysis = self.analyze_statistical_test(test, test_method,endpoint, True, **worker_args)
        ...
        if requires_correction:
            ...
            for test_index, test_item in enumerate(test_suite_result.results):
                if test_item.result.original_test_statistics:  # result can be None if not enough entities
                    ...
                    corrected_analysis = self.analyze_statistical_test(test_item.test, test_method,endpoint,
